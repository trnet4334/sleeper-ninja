...
export async function handler(
  ...
  const refreshCookieHeader: string | null = null;
  if (isTokenExpired(token)) {
    try {
      ...
      // refreshCookieHeader will be set in the response below
      return new Response(
        JSON.stringify({ status: "ok", leagueId, transactions: [] }),
        {
          status: 200,
          headers: {
