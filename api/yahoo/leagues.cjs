...
__export(leagues_exports, {
  handler: () => handler,
  parseYahooLeagues: () => parseYahooLeagues
});
...
function parseYahooLeagues(data) {
  try {
    ...
    const userEntry = users?.["0"]?.user;
    if (!Array.isArray(userEntry) || userEntry.length < 2) return [];
    const games = userEntry[1]?.games;
    const gameEntry = games?.["0"]?.game;
    if (!Array.isArray(gameEntry) || gameEntry.length < 2) return [];
    const leaguesObj = gameEntry[1]?.leagues;
    if (!leaguesObj || typeof leaguesObj !== "object") return [];
    const count = Number(leaguesObj?.count ?? 0);
    const results = [];
    for (let i = 0; i < count; i++) {
      const entry = leaguesObj[String(i)];
      const leagueArr = entry?.league;
      ...
      results.push({
        ...
        season: parseInt(meta.season ?? String((/* @__PURE__ */ new Date()).getFullYear()), 10)
      });
    }
    return results;
  } catch {
...
async function fetchYahooLeagues(accessToken) {
  const url = "https://fantasysports.yahooapis.com/fantasy/v2/users;use_login=1/games;game_keys=mlb/leagues?format=json";
