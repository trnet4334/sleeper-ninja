...
// api/data/matchup.ts
var matchup_exports = {};
__export(matchup_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(matchup_exports);
...
async function matchupSummaryAsync(query) {
  ...
  const pickups = (await queryPlayersWithFallback(
  ...
  const forecast = categories.map((category, index) => ({
    ...
    opponentValue: query.playerType === "pitcher" ? Number((3 + index * 0.24).toFixed(2)) : 9 + index,
...
// api/data/matchup.ts
async function handler(request = new Request("http://localhost/api/data/matchup")) {
